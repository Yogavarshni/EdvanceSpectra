...
from RephraserLLM.rephraserllm import generate_prompt_from_list
...
def create_agent_server(
    ...
    @app.post("/rephrase", response_model=AgentResponse)
    async def rephrase_and_run(request: RephraseRequest = Body(...)):
        try:
            resp = generate_prompt_from_list(request.message)
            result = await task_manager.process_task(resp, request.context, request.session_id)
...
#     """Standard A2A agent response format."""
#     message: str = Field(..., description="The response message")
#     path: str = Field(..., description="Generated materials")
#     # status: str = Field(default="success", description="Status of the response (success, error)")
#     # data: Dict[str, Any] = Field(default_factory=dict, description="Additional data returned by the agent")
#     # session_id: Optional[str] = Field(None, description="Session identifier for stateful interactions")
